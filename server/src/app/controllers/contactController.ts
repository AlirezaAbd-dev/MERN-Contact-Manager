...
export const editContact: RequestHandler = async (req, res) => {
    ...
    contactsModel.findOne({_id: id}).then(async contact => {
        const {error} = editContactsValidator(req.body)
        if (error) res.status(400).send({message: 'لطفا تمام فیلد ها را پر کنید!'})
...
export const addContact: RequestHandler = async (req, res) => {
    const {error} = editContactsValidator(req.body)
    if (error) return res.status(400).send({message: 'لطفا تمام فیلد ها را پر کنید!'})
    let {fullName} = req.body

    let contact = await contactsModel.findOne({fullName: fullName})
    if (contact) return res.status(400).send({message: 'این نام کاربری در دسترس نیست!'})
...
export const getOneContact:RequestHandler = async (req, res) => {
    ...
    if (!contact) return res.status(404).send({message:'مخاطب مورد نظر یافت نشد!'})
